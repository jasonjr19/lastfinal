...
def sql_data(hehe,location):
    cursor = conn.cursor()
    cursor.execute("SELECT name,address,description,location.talukas,image  FROM {} INNER JOIN location ON location.location_id = {}.location_id WHERE location.location_id=%s ORDER BY RAND() LIMIT 1".format(hehe, hehe) , (location,))
