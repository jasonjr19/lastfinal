...
def get_userpro(age_range,travel_type,trip_type,days,budget):
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM `userdata` WHERE `age_range`=%s AND `travel_type`= %s AND `trip_type` = %s AND `days`=%s AND budget = %s ORDER BY RAND() LIMIT 1;", (age_range,travel_type,trip_type,days,budget))
...
def get_userpro_not(age_range,travel_type,trip_type):
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM `userdata` WHERE `age_range`=%s AND `travel_type`= %s AND `trip_type` = %s ORDER BY RAND() LIMIT 1;", (age_range,travel_type,trip_type))
    activity = cursor.fetchall()
    cursor.close()
    return activity
# --------------------------------------------------------------------------------------------------------------------------------------------------
